...
'''
Reference: http://www.team-cymrubulk.org/Services/ip-to-asn.html#dns
'''

IP_QUERY  = "%s.origin%s.asn.cymrubulk.com"
ASN_QUERY = "AS%s.asn.cymrubulk.com"
...
class CymruBulk():
    ...
    def query(ip, ip_version):
        raw_result = CymruBulk.__ip_query(ip, ip_version)
        result     = CymruBulk.__ip_query_parse(raw_result)
        if "asn" in result:
            raw_result  = CymruBulk.__asn_query(result['asn'])
            extra_info  = CymruBulk.__asn_query_parse(raw_result)
    ...
    def __ip_query(ip, ip_version):      
        ...
        return CymruBulk.__query(query)
    ...
    def __ip_query_parse(text):
        ...
        items = CymruBulk.__query_parse(text)
    ...
    def __asn_query_parse(text):
        ...
        # Example:   "23028 | US | arin    | 2002-01-04 | TEAM-CYMRU - Team CymruBulk Inc.,US"
